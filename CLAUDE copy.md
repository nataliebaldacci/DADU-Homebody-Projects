# CASTLEHOLD / HOMEBODY BUILDER â€” CLAUDE CODE INSTRUCTIONS
**Version:** 3.0 | **Date:** February 12, 2026
**Author:** Natalie Baldacci, Vanderbilt Law School J.D. 2026
**Repo:** ~/Desktop/Master_Data/DADU/DADU_Homebody/GitHub_Repo/
...
## 2. CRITICAL RULES â€” READ BEFORE TOUCHING ANYTHING
...
## 3. BRAND SYSTEM (LOCKED â€” DO NOT CHANGE)

### Color Palette
```css
:root {
  --navy: #3A5566;           /* Deep Slate â€” primary, headings, nav */
  --navy-dark: #2E4553;      /* Darker Slate */
  --navy-light: #4A6B7D;     /* Lighter Slate */
  --teal: #7B746D;           /* Warm Stone â€” secondary UI, subtitles */
  --teal-light: #918A83;     /* Lighter Stone */
  --terracotta: #C58B2A;     /* Ochre â€” accent, use sparingly */
  ...
  --error: #7A2A1D;          /* Oxide Red â€” restrictions/legal only */
  --status-eligible: #2E6F4E; /* Green â€” eligibility UI states only */
}
```
...
- SVG icon versions in `~/Desktop/Master_Data/DADU/All_Icons/Icons/SVG_Originals/`

---

## 4. NAVIGATION STRUCTURE (LOCKED â€” DO NOT CHANGE)

### Top Bar Layout
```
LOGO | EXPLORE â–¾ | BUILD â–¾ | DATA â–¾ | PRICING | ðŸ” Search | My Projects | Get Started â†’
```

### Routing
- LOGO â†’ index.html
- Search ðŸ” â†’ property_search.html
- My Projects â†’ project_planner.html (placeholder for now)
- Get Started â†’ am_i_eligible.html
...
**NO "Contractor Finder" â€” use Contractor Marketplace only.**
...
## 5. CURRENT STATE â€” WHAT EXISTS

### Shared Components (Created by Claude Code)
- `homebody_shared.css` â€” Global stylesheet with Castlehold :root variables and mega-menu CSS
- `homebody_header.js` â€” Shared nav component injected via `<div id="site-header"></div>`
- `homebody_header.html` â€” Shared header template
...
## 6. WHAT NEEDS TO BE FINISHED â€” PRIORITY ORDER
...
4. Test the eligibility map (dadu_eligibility_map.html) â€” this is the most important map
...
1. Implement all external link builders using correct field mappings:
   ...
   ```
2. Use `assessor_accounts_20260114.csv` to map APN â†’ ACCOUNTNUMBER for property card links
...
4. Other contractor fields are mostly empty â€” do not rely on them without verification
...
- am_i_eligible.html â€” Address lookup with instant eligibility determination
- user-homeowners.html â€” Homeowner portal landing page
- contractor_marketplace.html â€” Connected to real permit data
- property_search.html â€” Map-based search like Regrid
...
| assessor_accounts_20260114.csv | â€” | Maps APN to ACCOUNTNUMBER | Local |
...
Use `assessor_accounts_20260114.csv` to bridge APN â†” ACCOUNTNUMBER.
...
- `/GetPermitHistory?apn={APN}` â€” All permits for parcel
- `/GetGenInfo?pin={PIN}` â€” General parcel info
- `/GetOwnerHistory?pin={PIN}` â€” Ownership chain
- `/GetZoningHistory?pin={PIN}` â€” Zoning changes
...
| Lot â‰¥ 10,000 SF | Max 850 SF living / 1,000 SF footprint |
...
â”œâ”€â”€ index.html                    # Homepage (Castlehold branded)
â”œâ”€â”€ homebody_shared.css           # Global stylesheet
â”œâ”€â”€ homebody_header.js            # Shared nav component
â”œâ”€â”€ homebody_header.html          # Header template
â”œâ”€â”€ /assets/
â”‚   â”œâ”€â”€ /css/                     # Additional stylesheets
â”‚   â”œâ”€â”€ /js/                      # Shared JavaScript modules
â”‚   â”‚   â””â”€â”€ arcgis-services.js    # ArcGIS query utilities
â”‚   â””â”€â”€ /icons/                   # PNG icons for nav/cards
â”œâ”€â”€ /data/
â”‚   â”œâ”€â”€ gis_data_sources.json     # ArcGIS endpoint reference
â”‚   â”œâ”€â”€ docs_index.json           # Document portal index (TO CREATE)
â”‚   â””â”€â”€ parcels_docs_summary.json # Per-parcel doc summary (TO CREATE)
â”œâ”€â”€ /docs/                        # PDF documents
â”œâ”€â”€ /samples/                     # Design reference (DO NOT MODIFY)
â”œâ”€â”€ /sample_reports/              # Sample report previews
â””â”€â”€ [95+ HTML pages at root]
```
...
Also at: `~/Desktop/Master_Data/DADU/All_Icons/Icons/SVG_Originals/`

---

## 12. AUDIT COMMANDS â€” RUN THESE FIRST

```bash
cd ~/Desktop/Master_Data/DADU/DADU_Homebody/GitHub_Repo/
...
~/Desktop/Master_Data/DADU/DADU_Homebody/GitHub_Repo/  # Git repo
```

### Script Output Conventions
- Save scripts to: `~/Desktop/Master_Data/Scripts/`
- Save data outputs to: `~/Desktop/Master_Data/DADU/[new_folder_name]/`
...
1. **Short plan** â€” What you will build and why
2. **Exact file paths** â€” Full paths relative to repo root
3. **Complete code** for each file you touch â€” no partial snippets
4. **Notes on icons** â€” Where to place them and expected filenames
5. **GitHub Pages test checklist** â€” Steps to verify everything works
...
Step 2: Fix branding â€” replace all old colors, ensure shared CSS loaded everywhere
Step 3: Fix shared header â€” verify locked nav structure on all pages
Step 4: Fix maps â€” debug blank rendering, connect data layers
Step 5: Wire Property Report Card â€” all external link builders working
Step 6: Document portal â€” build docs_index.json, enable search
Step 7: Contractor marketplace â€” connect to real permit data
Step 8: Near Me locator â€” consolidate to one working version
Step 9: Fill placeholder pages â€” am_i_eligible, homeowner portal, etc.
Step 10: Test all nav links â€” ensure every href target exists
```
